...
//////////////////////////////////////////////////////////////////////////
std_msgs::Int16 pwmLeftReq_data;
ros::Publisher reqPWMPubLeft("requiredPWM_left", &pwmLeftReq_data);

std_msgs::Int16 pwmLeftOut_data;
ros::Publisher outPWMPubLeft("PWMout_Left", &pwmLeftOut_data);

std_msgs::Int16 pwmRightReq_data;
ros::Publisher reqPWMPubRight("requiredPWM_Right", &pwmRightReq_data);

std_msgs::Int16 pwmRightOut_data;
ros::Publisher outPWMPubRight("PWMout_Right", &pwmRightOut_data);
...
const int TICKS_PER_REVOLUTION = 370;
...
const double TICKS_PER_METER = 982; // Originally 2880
 
// Proportional constant, which was measured by measuring the 
// PWM-Linear Velocity relationship for the robot.
const int K_P = 278;   //278
 
// Y-intercept for the PWM-Linear Velocity relationship for the robot
const int b = 33;
 
// Correction multiplier for drift. Chosen through experimentation.
const int DRIFT_MULTIPLIER = 120;    //120
 
// Turning PWM output (0 = min, 255 = max for PWM values)
const int PWM_TURN = 255;
 
// Set maximum and minimum limits for the PWM values
const int PWM_MIN = 35; // about 0.1 m/s
...
void setup() {
  ...
  nh.advertise(reqPWMPubLeft);
  nh.advertise(reqPWMPubRight);
  nh.advertise(outPWMPubLeft);
  nh.advertise(outPWMPubRight);
...
void loop() {
  ...
  pwmLeftReq_data.data = pwmLeftReq;
  pwmRightReq_data.data = pwmRightReq;
  pwmRightOut_data.data = pwmRightOut;
  pwmLeftOut_data.data = pwmLeftOut;
  ...
  if (currentMillis - previousMillis > interval) {
    ...
    reqPWMPubLeft.publish( &pwmLeftReq_data );
    outPWMPubLeft.publish( &pwmLeftOut_data );
    reqPWMPubRight.publish( &pwmRightReq_data );
    outPWMPubRight.publish( &pwmRightOut_data );
